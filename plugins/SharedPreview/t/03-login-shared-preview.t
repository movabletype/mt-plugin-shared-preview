...
my ( $app1, $need_password_spid )
    = SharedPreviewTest::make_shared_preview($super_author, $blog1->id, $entry1->id);
my ( $app2, $no_password_spid ) = SharedPreviewTest::make_shared_preview($super_author, $blog2->id, $entry2->id);
...
subtest 'login_shared_preview' => sub {
    subtest 'Not need login' => sub {
        ...
        SharedPreviewTest::check_redirect_shared_preview($output, $no_password_spid);
    };
    ...
    subtest 'Need login' => sub {
        ...
        subtest 'Enter password and login' => sub {
            subtest 'Login successful' => sub {
                my $output = post_request_login($need_password_spid, 'test', '');
                ok($output, 'Output') or return;
                SharedPreviewTest::check_redirect_shared_preview($output, $need_password_spid);
            };
            ...
            subtest 'Login failure' => sub {
                subtest 'Password not entered' => sub {
                    ...
                    ok($output, 'Output') or return;
                    check_login($output, $blog1, $need_password_spid, 'You must supply a password.');
                };

                subtest 'Password mismatch' => sub {
                    my $output = post_request_login($need_password_spid, 'mismatch', '');
                    ok($output, 'Output') or return;
                    check_login($output, $blog1, $need_password_spid, 'Passwords do not match.');
                };

                subtest 'Failed to create session' => sub {
                    my $module = Test::MockModule->new('SharedPreview::Auth');
                    $module->mock( 'start_session', sub {
                        return 'Failed to create session';
                    });

                    my $output = post_request_login($need_password_spid, 'test', '');
                    ok($output, 'Output') or return;
                    check_login($output, $blog1, $need_password_spid, 'Failed to create session');
                };
            };
        };
    };
...
sub check_login {
    ...
    like( $output, qr/In order to view shared preview of <a href="$site_url">$site_name<\/a>, password is required\./,
        'login message'
    );

    like( $output, qr/This password is different from your login password\./,
        'password message'
    );

    like( $output, qr/Remember me\?/,
        'Remember message'
    );

    if ($message) {
        like( $output, qr/$message/,
            'error message'
        );
    }
}
