...
my $objs = MT::Test::Fixture->prepare(
    {   author => [
        ...
        blog  => [ { name => $blog1_name, },{ name => $blog2_name, }, { name => $blog3_name, }, { name => $blog4_name, }],
    }
);
...
subtest 'shared preview setting' => sub {
    ...
    subtest 'save shared preview setting' => sub {
        subtest 'one password' => sub {
            my @password = ('saved1');
            my $output = post_setting($blog1->id, \@password, 1, $super_author);
            ok($output, 'Output') or return;
        ...
        subtest 'two passwords' => sub {
            ...
            my $output = post_setting($blog3->id, \@password, 1, $super_author);
            ok($output, 'Output') or return;
    ...
    subtest 'timeout' => sub {
        ...
        ok($output, 'Output') or return;
        like($output, qr/(<input.*type="hidden".*name="plugin_sig".*value="SharedPreview")/, 'plugin_sig');
        like($output, qr/(<input.*type="hidden".*name="blog_id".*value="$blog_id")/, 'blog_id');
        like($output, qr/(<input.*type="hidden".*name="sp_password\[\]".*value="saved1")/, 'shared preview password');
        like($output, qr/(<input.*type="hidden".*name="__mode".*value="save_plugin_config")/, 'plugin_sig');

        unlike($output, qr/(<input type="text" name="username" id="username".*value="saved1".*\/>)/, 'username is empty');
        unlike($output, qr/(<input type="password" name="password" id="password".*value="saved1".*\/>)/, 'password is empty');
    }
...
sub check_setting {
    ...
    like($output, qr/(Enable to share preview of entry, page and content data\.)/, 'Explanatory text');
    like($output, qr/(<input.*name="sp_password\[\]")/, 'text box');
    like($output, qr/(<a href="javascript:void\(0\);".*id="add_password".*>.*Add Password\.<\/a>)/, 'add password');
}
...
sub check_saved_setting {
    ...
    if (ref($plugin_data->data->{'sp_password[]'}) eq 'ARRAY') {
        foreach my $value (@{$passwords}) {
            ok (grep { $_ eq $value } @{$plugin_data->data->{'sp_password[]'}} , 'password registered' . $value);
        }

    } else {
        ok ( $passwords->[0] eq $plugin_data->data->{'sp_password[]'}, 'password registered' );
    }
}
