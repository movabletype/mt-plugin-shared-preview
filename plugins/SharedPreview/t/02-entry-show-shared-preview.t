...
subtest 'show_shared_preview' => sub {
    subtest 'need to login' => sub {
        ...
        subtest 'has cookie' => sub {
            subtest 'Password match in session' => sub {
                ...
                my $output = SharedPreviewTest::request_show_shared_preview($spid);
                ok( $output, 'Output' ) or return;
                SharedPreviewTest::check_shared_preview( $output, $blog1, $entry1, $spid );
            };
            ...
            subtest 'Password mismatch in session' => sub {
                ...
                my ( $app, $spid )
                    = SharedPreviewTest::make_shared_preview( $super_author, $blog_id,
                    $entry_id );
    ...
    subtest 'no need to login' => sub {
        my ( $app, $spid )
            = SharedPreviewTest::make_shared_preview( $super_author, $blog2->id, $entry2->id );

        my $output = SharedPreviewTest::request_show_shared_preview($spid);
        ok( $output, 'Output' ) or return;
        SharedPreviewTest::check_shared_preview( $output, $blog2, $entry2, $spid );
    };
...
sub check_redirect_login {
    ...
    like( $output , qr/Location: .*(__mode=shared_preview_login).*(spid=$spid)/,
