...
subtest 'entry page' => sub {
    subtest 'create new entry' => sub {
        my $output = request_entry_page( $super_author, $blog1->id, '', 'entry', '' );
        ok($output, 'Output') or return;
        unlike($output, qr/shared-preview-widget/, 'shared preview widget');
        unlike($output, qr/__mode=make_shared_preview/, 'shared preview link');
        unlike($output, qr/Open the shared preview/, 'shared preview label');
    };

    subtest 'edit entry' => sub {
        subtest 'published' => sub {
            my $output = request_entry_page( $super_author, $blog1->id, $entry3->id, 'entry', '' );
            ok($output, 'Output') or return;
            unlike($output, qr/shared-preview-widget/, 'shared preview widget');
            unlike($output, qr/__mode=make_shared_preview/, 'shared preview link');
            unlike($output, qr/Open the shared preview/, 'shared preview label');
        ...
        subtest 'unpublished' => sub {
            my $output = request_entry_page( $super_author, $blog1->id, $entry1->id, 'entry', '' );
            ok($output, 'Output') or return;

            like($output, qr/shared-preview-widget/, 'shared preview widget');
            like($output, qr/__mode=make_shared_preview/, 'shared preview link');
            like($output, qr/Open the shared preview/, 'shared preview label');

            my ($make_shared_preview_link) = $output =~ m/jQuery\('#entry-publishing-widget'\)\.before.*href=\\"(.*?)\\"\s/;
            ok($make_shared_preview_link, 'make shared preview link') or return;

            my $uri = URI->new($make_shared_preview_link);
            is ($uri->query_param('__mode') ,'make_shared_preview', 'correct mode');
    ...
    subtest 'save entry' => sub {
        subtest 'unpublished to published' => sub {
            my ($app, $make_preview) = SharedPreviewTest::make_shared_preview( $super_author, $blog1->id, $entry2->id) or return;
            my $output = request_save_entry($super_author, $blog1->id, $entry2, 2, 0);
            ok($output, 'Output') or return;
    ...
    subtest 'saved added entry page' => sub {
        subtest 'published' => sub {
            my $output = request_entry_page( $super_author, $blog1->id, $entry3->id, 'entry', 'added' );
            ok($output, 'Output') or return;

            my ($make_shared_preview_link) = $output =~ m/jQuery\('#saved-added'\)\.append.*href=\\"(.*?)\\"/;
            ok(!$make_shared_preview_link, 'shared preview link');
        };

        subtest 'unpublished' => sub {
            my $output = request_entry_page( $super_author, $blog1->id, $entry1->id, 'entry', 'added' );
            ok($output, 'Output') or return;
    ...
    subtest 'saved changes entry page' => sub {
        subtest 'published' => sub {
            my $output = request_entry_page( $super_author, $blog1->id, $entry3->id, 'entry', 'changes' );
            ok($output, 'Output') or return;

            my ($make_shared_preview_link) = $output =~ m/jQuery\('#saved-changes'\)\.append.*href=\\"(.*?)\\"/;
            ok(!$make_shared_preview_link, 'shared preview link');

        };

        subtest 'unpublished' => sub {
            my $output = request_entry_page( $super_author, $blog1->id, $entry1->id, 'entry', 'changes' );
            ok($output, 'Output') or return;
...
sub check_response_make_shared_preview {
    ...
    like( $output , qr/Location: .*mt-shared-preview.cgi/,'is shared preview url');
