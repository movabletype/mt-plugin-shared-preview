...
sub make_shared_preview {
    ...
    if (my $preview = MT::Preview->load({
        ...
        $preview_obj->save
            or $app->error("Could not create share preview link : " . $preview_obj->errstr);
    }
...
sub set_save_values {
    ...
    for my $column (keys $set_data) {
        ...
        if ($column_name eq 'data') {
            $data->{$set_data->[$column]{data_name}} = $set_data->[$column]{data_value};
        }
        elsif ($column_name) {
...
sub shared_preview {
    ...
    if (MT::PreviewSetting->load({ blog_id => $preview->blog_id, use_password => 0 })) {
        # $app->redirect();
    }
